...
class Main extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->initializeExtendedSession();
     ...
     */
    private function initializeExtendedSession()
    {
    ...
    public function home()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Log session health
            $this->logSuccess("Session check - ID: " . $session_id . ", Time diff: " . $timediff . " seconds");
    ...
    /**
     * Enhanced validateSession method with better error handling
     */
    private function validateSession($session_id)
    {
        try {
            ...
            if (empty($checksession) || $checksession[0]['session_id'] !== $session_id) {
                return [
                    'valid' => false,
                    'message' => 'Invalid session ID or user not logged in',
                    'data' => null
                ];
            }

            // Check session timeout with extended window
            $loggedtime = $checksession[0]['created_on'];
            $currentTime = $this->date;
            $loggedTimestamp = strtotime($loggedtime);
            $currentTimestamp = strtotime($currentTime);
    ...
    /**
     * Updated DepositToDeriv method to handle session properly
     */
    public function DepositToDeriv()
    {
        // Validate request method
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            $this->sendErrorResponse('Only POST request allowed', 400);
            return;
        }
        ...
        // Validate session and store session data
        $sessionValidation = $this->validateSession($session_id);
        if (!$sessionValidation['valid']) {
            $this->sendErrorResponse($sessionValidation['message'], 401);
            return;
        }

        // Store session data for later use
        $sessionData = $sessionValidation['data'];
        $wallet_id = $sessionData['wallet_id'];

        // Process deposit with session data passed explicitly
        try {
            $transferResult = $this->processDerivDepositFixed(
                $crNumber,
                $amount,
                $wallet_id,
                $transaction_id,
            ...
            $this->sendSuccessResponse($transferResult, $crNumber, $amount, $wallet_id);
     ...
     /**
    * Fixed process Deriv deposit with explicit session handling
    */
    private function processDerivDepositFixed($crNumber, $amount, $wallet_id, $transaction_id, $session_id)
    {
        // Get conversion rates
        $buyrate = $this->getBuyRate();
        $amountUSD = round($amount / $buyrate['kes'], 2);
        ...
        // Check user balance
        $userBalance = $this->getUserBalance($wallet_id);
        if ($userBalance < $amount) {
            throw new Exception('Insufficient funds. Your balance is KSH ' . number_format($userBalance, 2));
        }

        // Check duplicate transaction
        $this->checkDuplicateTransaction($transaction_id);

        // Start database transaction
        $this->db->trans_start();

        try {
            // Execute Deriv transfer with session data
            $transferResult = $this->executeDerivTransferFixed($crNumber, $amountUSD, $session_id);
            ...
            // Update session timestamp at the end
            $this->updateSessionTimestampFixed($session_id);
    ...
    /**
    * Fixed Deriv transfer execution without session interference
    */
    private function executeDerivTransferFixed($loginid, $amount, $session_id)
    {
        try {
            $appId = 76420;
            ...
            // Create WebSocket client with proper timeout
            ...
            // Check balance
            if (!isset($authResponse['authorize']['balance']) || 
                $authResponse['authorize']['balance'] < $amount) {
                throw new Exception('Insufficient payment agent balance');
            }
            ...
            if (isset($transferResponse['error'])) {
                throw new Exception($transferResponse['error']['message']);
            }

            if (!isset($transferResponse['paymentagent_transfer']) || 
                $transferResponse['paymentagent_transfer'] != 1) {
            ...
            $this->logError("Transfer failed for $loginid: " . $e->getMessage());
            return [
                'success' => false,
    ...
    private function getBuyRate()
    {
        $buyratecondition = ['exchange_type' => 1, 'service_type' => 1];
    ...
    private function getUserBalance($wallet_id)
    {
        $summary = $this->Operations->customer_transection_summary($wallet_id);
    ...
    private function checkDuplicateTransaction($transaction_id)
    {
        $duplicate_check = $this->Operations->SearchByCondition(
            'deriv_deposit_request',
            ['transaction_id' => $transaction_id, 'status' => 1]
        );
    ...
    private function recordTransaction($wallet_id, $transaction_id, $crNumber, $amount, $amountUSD, $buyrate)
    {
        $current_time = date('Y-m-d H:i:s');
        $transaction_number = $this->transaction_number;
        $mycharge = ($buyrate['kes'] - $buyrate['bought_at']);
        ...
        $this->Operations->Create('customer_ledger', $ledger_data);
        $this->Operations->Create('system_ledger', $ledger_data);
    }

    /**
     * Updated sendSuccessResponse with explicit wallet_id
     */
    private function sendSuccessResponse($transferResult, $crNumber, $amount, $wallet_id)
    {
        $message = 'Deposit of $' . number_format($amount, 2) . ' USD successfully completed to Deriv account ' . $crNumber;
        ...
        if ($phone) {
            ...
            $this->Operations->sendSMS('0703416091', "Deposit completed: $" . number_format($amount, 2) . " USD to " . $crNumber);
        }
    ...
    /**
     * Updated process_request method with better session handling
     */
    public function process_request($request_id)
    {
        ...
        // Use the fixed transfer method without session updates
        $transferResult = $this->transferToDerivAccountFixed($cr_number, $amount);
    ...
    /**
     * Simplified transfer method for webhook contexts (no session updates)
     */
    private function transferToDerivAccountFixed($loginid, $amount)
    {
        try {
            $appId = 76420;
            $url = "wss://ws.derivws.com/websockets/v3?app_id={$appId}";
            ...
            if (isset($transferResponse['paymentagent_transfer']) && $transferResponse['paymentagent_transfer'] == 1) {
   ...
   /**
     * Fixed session timestamp update with explicit session_id
     */
    private function updateSessionTimestampFixed($session_id)
    {
        try {
            if (empty($session_id)) {
            ...
            $result = $this->Operations->UpdateData($session_table, $session_condition, $update_data);
            
            if (!$result) {
                throw new Exception('Failed to update session timestamp');
            }
            
            $this->logSuccess("Session updated successfully for: $session_id");
            return true;
            
        } catch (Exception $e) {
            $this->logError('Session update failed: ' . $e->getMessage());
            return false;
        }
    }
    
    private function recoverSession($session_id)
    {
        $session_data = $this->Operations->SearchByCondition('login_session', ['session_id' => $session_id]);
        if (!empty($session_data)) {
            $this->session->set_userdata([
                'session_id' => $session_id,
                'wallet_id' => $session_data[0]['wallet_id'],
                'initialized' => true
            ]);
            return true;
        }
    ...
    public function WithdrawFromDeriv()
    {
        ...
        if (!$sessionValidation['valid']) {
            $response['status'] = 'fail';
            $response['message'] = $sessionValidation['message'];
            $response['data'] = null;
            echo json_encode($response);
            exit();
        }

        $wallet_id = $sessionValidation['data']['wallet_id'];

        // Validate minimum withdrawal amount
        if ($amount < 2.5) {
            $response['status'] = 'error';
            $response['message'] = 'The minimum withdrawal amount is $2.50 USD.';
            $response['data'] = null;
            echo json_encode($response);
            exit();
        }
        ...
        $pending_withdrawals = $this->Operations->SearchByCondition(
            'deriv_withdraw_request',
            array('wallet_id' => $wallet_id, 'status' => 0)
        );

        if (!empty($pending_withdrawals)) {
            $response['status'] = 'error';
            $response['message'] = 'You have a pending withdrawal request. Please wait for it to be processed.';
            $response['data'] = null;
            echo json_encode($response);
            exit();
        }
        ...
        $save = $this->Operations->Create('deriv_withdraw_request', $withdrawal_data);
        ...
        if ($save === TRUE) {
            ...
            // Send notifications
            $this->Operations->sendSMS($phone, $message);
            $this->Operations->sendSMS('0703416091', "New withdrawal request: $" . $amount . " USD from " . $crNumber);

            $response['status'] = 'success';
            $response['message'] = $message;
            $response['data'] = $withdrawal_data;
        } else {
            $response['status'] = 'error';
